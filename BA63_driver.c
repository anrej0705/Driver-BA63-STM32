...
void BA63_SetCP(uint16_t cpCode)
{
	switch(cpCode)
	{
		...
		{
			cpcode[2]=0x35;
			HAL_UART_Transmit(&huart1, (uint8_t*)cpcode, sizeof(cpcode), 0xFFFF);
			break;
		}
		default:
		{
...
{
	HAL_UART_Transmit(&huart1, (uint8_t*)clearCmd, sizeof(clearCmd), 0xFFFF);
}

//Задачем позицию курсора
void BA63_SetPos(uint8_t _pX, uint8_t _pY)
{
	...
	setCurPos[4] = _X[0];
	setCurPos[2] = _Y[0];
	HAL_UART_Transmit(&huart1, (uint8_t*)setCurPos, sizeof(setCurPos), 0xFFFF);
}
...
void BA63_SendString(uint8_t *string)
{
	...
	while(string[counter]!=0)
	{
		...
		HAL_UART_Transmit(&huart1, &charToSend, 1, 0xFFFF);
		counter++;
	}
}
...
{
	HAL_UART_Transmit(&huart1, (uint8_t*)delEndline, sizeof(cpcode), 0xFFFF);
}
